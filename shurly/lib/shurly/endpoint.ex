defmodule Shurly.Endpoint do
  @moduledoc """
  A Plug responsible for logging request info, (de)serializing data, matching
  routes, and dispatching responses.
  """
  ...
  match "/v1/url/:slug", via: :get do
    {status, body} = if Shurly.Resolver.is_valid_looking_slug?(slug) do
  ...
  match ":slug", via: :get do
    {conn, status, body} = if Shurly.Resolver.is_valid_looking_slug?(slug) do
                             if url = Shurly.Resolver.resolve_slug(slug) do
                               {put_resp_header(conn, "location", url), Shurly.Config.redirect_code, "Redirecting to #{url}"}
                             else
